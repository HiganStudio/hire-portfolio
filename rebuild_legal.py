...
# Fix paths in prefix and suffix
def fix_template(html):
    ...
    # Replace onClick handlers with hrefs back to main page
    ...
    # Remove onClick attributes from span tags and convert to anchor tags if necessary
    html = html.replace('<span class="hg-navlink"', '<a class="hg-navlink"')
    html = html.replace('</span>', '</a>') # This is a bit unsafe globally, but wait
    return html

prefix = fix_template(prefix)
suffix = fix_template(suffix)

# Let's write a safer replace for span to a:
def safe_span_replace(html):
    # Only replace spans that have class="hg-navlink" or "hg-guide"
    html = re.sub(r'<span ([^>]*class="[^"]*(hg-navlink|hg-guide)[^"]*"[^>]*)>', r'<a \1>', html)
    html = html.replace('</a>', '</a>') # Already a, but we need to close the ones we opened.
    # Actually wait, replacing closing spans is hard because they don't have classes.
    # It's better to just inject hrefs into the elements directly. Since onClick is replaced by href, if it's a span, it won't be clickable as a link in standard HTML (though JS might intercept it).
    # To fix this, let's just do a specific regex replace for the nav buttons in header/footer/nav.
...
css = """
  ...
  .legal-content .disclaimer-box { background: rgba(123,34,144,0.06); border: 1px solid rgba(180,100,220,0.15); border-radius: 12px; padding: 24px 32px; margin-bottom: 48px; text-align: center; }
  .legal-content .disclaimer-title { font-family: 'Space Mono', monospace; font-size: 11px; letter-spacing: 0.15em; color: #d9c6ee; margin-bottom: 12px; text-transform: uppercase; font-weight: 600; }
