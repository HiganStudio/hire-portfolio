...
class Component extends DCLogic {
  ...
  componentWillUnmount(){ removeEventListener('scroll',this._onScroll); cancelAnimationFrame(this._raf); cancelAnimationFrame(this._fallRaf); }
  ...
  componentDidUpdate(_,prev){
    const routeChanged = prev && prev.route!==this.state.route;
    if(routeChanged){ this.setState({typed:'',wordIdx:0}); requestAnimationFrame(()=>{ window.scrollTo(0,0); this._sig=null; this.mountSlots(); this.onScroll(); setTimeout(()=>{ this._sig=null; this.mountSlots(); },90); }); return; }
  ...
  renderVals(){
    ...
    return {
      ...
      rootRef:el=>this._root=el, navRef:el=>this._nav=el, footerRef:el=>this._footer=el,
